...
def loss_batch(
    ...
    if opt is not None and pipeline_handler is not None:
...
def validate(
    ...
    with torch.no_grad():
        ...
        for xb, yb in progress_bar(dl, parent=pbar, leave=(pbar is not None)):
            if pipeline_handler:
                pipeline_handler.on_batch_begin(
                    x=xb, y=yb, ru_mode=RunMode.TRAIN
                )
            val_loss = loss_batch(
                model, xb, yb, loss_func, pipeline_handler=pipeline_handler
            )
